...
public class ProductResource {
	...
	return ResponseEntity.status(HttpStatus.CREATED).body(service.save(product));
		...
		/*HttpHeaders myheaders = new HttpHeaders();//use springboot package
		myheaders.add("secret", "fooboo");
		return ResponseEntity.status(HttpStatus.CREATED).headers(myheaders).body(service.save(product)); */
		  //return new ResponseEntity<Product>("Hello World", responseHeaders, HttpStatus.CREATED);
	  }
